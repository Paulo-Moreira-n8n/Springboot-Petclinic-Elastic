...
class StateSearch():
    ...
    def query(self):
        ...
        # AFTER (template explícito)
        query = json.loads(self.query_renderer.render_path('state_search.mustache', self))
...
class CitySearch():
    ...
    def query(self):
        query = json.loads(self.query_renderer.render_path('city_search.mustache', self))
        results = current_app.elasticsearch.search(index=current_app.config['ADDRESS_INDEX'], body=query)
        cities = [result["key"] for result in results["aggregations"]["city"]["buckets"]]
        # Sem fallback por state: a API agora usa somente zip_code como chave
        return {
            "cities": list({city.lower() for city in cities})
        }
...
class AddressSearch():
    ...
    def query(self):
        query = json.loads(self.query_renderer.render_path('address_search.mustache', self))
        results = current_app.elasticsearch.search(index=current_app.config['ADDRESS_INDEX'], body=query)
