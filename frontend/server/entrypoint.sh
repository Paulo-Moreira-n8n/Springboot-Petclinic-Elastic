...
for source_map in /source_maps/*.map; do
  ...
  echo "ENV: APM-API=${ELASTIC_APM_API_URL} ES=${ELASTICSEARCH_URL} INT=${PETCLINIC_INTERNAL_URL} EXT=${PETCLINIC_EXTERNAL_URL}"
  ...
  echo "filename=${filename} bundle_js=${bundle_js}"
  ...
  response_code=$(curl -s -o /tmp/source_map.out -X POST "${ELASTIC_APM_API_URL}/api/apm/sourcemaps" -w "%{response_code}" \
    -F service_name="${ELASTIC_APM_SERVICE_NAME}" \
