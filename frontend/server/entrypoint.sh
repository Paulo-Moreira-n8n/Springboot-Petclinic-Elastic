...
# Espera apm-server
until  $(curl -s -o /tmp/apm_health_check.out --head --fail "${ELASTIC_APM_SERVER_URL:http://apm-server:8200}"); do
...
if [[ "${INSECURE_SSL}" == "true" ]]; then
  until curl -u "${ELASTICSEARCH_USERNAME:elastic}:${ELASTICSEARCH_PASSWORD:changeme}" -s "${ELASTICSEARCH_URL:http://elasticsearch:9200}/_cat/health" "--insecure" -o /dev/null; do
    echo "Waiting for Elasticsearch..."
    sleep 1
  done
else
  until curl -u "${ELASTICSEARCH_USERNAME:elastic}:${ELASTICSEARCH_PASSWORD:changeme}" -s "${ELASTICSEARCH_URL:http://elasticsearch:9200}/_cat/health" -o /dev/null; do
    echo "Waiting for Elasticsearch..."
    sleep 1
  done
fi
...
for source_map in source_maps/*.map; do
  ...
  response_code=$(curl -s -o /tmp/source_map.out -X POST "${ELASTIC_APM_API_URL}/api/apm/sourcemaps" \
    -u elastic:changeme \
	-w "%{response_code}" \
    -H 'Content-Type: multipart/form-data' \
	-H 'kbn-xsrf: true' \
    -F service_name="${ELASTIC_APM_SERVICE_NAME:petclinic-client-nodejs-service}" \
    -F service_version="${ELASTIC_APM_SERVICE_VERSION}" \
    -F bundle_filepath="${PETCLINIC_INTERNAL_URL:http://spring-petclinic-client:3000}/assets/${bundle_js}" \
...
#    exit 1
  ...
  echo "Loading source map $source_map with external URL ${PETCLINIC_EXTERNAL_URL:http://localhost:}/assets/${bundle_js}"
  response_code=$(curl -s -o /tmp/source_map.out -X POST "${ELASTIC_APM_API_URL}/api/apm/sourcemaps" \
    -u elastic:changeme \
    -w "%{response_code}" \
    -H 'Content-Type: multipart/form-data' \
